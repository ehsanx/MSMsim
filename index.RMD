...
100 simulation iterations were performed with 2,500 subjects in each simulation, with up to 10 followup
...
```{r}
round(mat.cox.adj.all$coef,2)
```

Coefs from MSM (stabilized weights used)
```{r}
round(mat.msm.all$coef,2)
```

## Print Summary Results

Results from unadjusted Cox regression
```{r}
summary(mat.cox.all$coef)
```

Results from adjusted Cox regression (adjusted by L)
```{r}
summary(mat.cox.adj.all$coef)
```

Results from MSM (unstabilized weights used)
```{r}
summary(mat.msm.w.all$coef)
```
...
Results from MSM (stabilized weights used)
```{r}
summary(mat.msm.all$coef)
```
